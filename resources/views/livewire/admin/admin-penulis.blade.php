<div class="content-wrapper">
  <div class="page-header">
    <h3 class="page-title">
      ...
      </span> Berita <i class="mdi mdi-chevron-double-right"></i><a href="">Penulis</a>
    </h3>
  </div>
<div>
          <!-- Row -->
      ...
      <div class="col-lg-8">
          <div class="card mb-4">
              <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Penulis Berita</h6>
              ...
              <div class="table-responsive p-3">
                  <table class="table table-bordered" id="dataTable">
                    ...
                    <tbody>
                           @foreach ($penulises as $no=>$penulis)
                      <tr>
                        <td>{{$no + $penulises ->firstItem() }}</td>
                        <td><img src="{{asset('assets/images/artikel')}}/{{$penulis->image }}" style="width: 60px; border-radius: 1px;"></td>
                        <td>{{$penulis->name}}</td>
                        <td>{{$penulis->blog_count}} Postingan</td>
                        <td>
                            <button wire:click="edit({{ $penulis->id }})" class="btn btn-sm btn-outline-primary" data-toggle="modal" data-target="#kategoriModal"><i class="fa fa-edit"></i> Edit</button> 
                            <botton onclick="confirm('serius ingin menghapus penulis Berita ....?') || event.stopImediatePropagation()" wire:click="delete({{ $penulis->id }})" class="btn btn-sm btn-outline-danger"><i class="fa fa-trash"></i> Hapus</botton>
                        </td>
                      </tr>
                       @endforeach
                    </tbody>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header" style="background-color: #0e5a12; color: white;">
                  <h5 class="modal-title" id="exampleModalLabelLogout">Edit Penulis</h5>
                ...
                <div class="modal-body">
                        ...
                        <div class="form-group">
                              <label for="validationServer01">Nama Penulis</label>
                               <input type="hidden" wire:model="penulis_id">
                              <input type="text" class="form-control is-valid" id="validationServer01" placeholder="Penulis name" required  wire:model="name" wire:keyup="generateSlug">
          ...
          <div wire:ignore.self class="modal fade" id="exmpleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelLogout"
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header" style="background-color: #0e5a12; color: white;">
                  <h5 class="modal-title" id="exampleModalLabelLogout">Tambah Penulis</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
